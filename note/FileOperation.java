...
public class FileOperation {
	...
	public static void main(String[] args) throws IOException {
		...
		FileController.fileAppend();
		
		
/*
		FileController.copyTextFile("data/DS_Sample1.txt", "data/DS_Sample2.txt");	
		FileController.copyTextFile("read.txt", "read.copy.txt");	
	
		FileController.copyBinaryFile("data/DS_Sample1.txt", "data/DS_SampleBin.txt");	
		FileController.copyBinaryFile("./read.txt", "./read.bin");

		FileController.fileDelete("data/DS_SampleBin.txt");
...
//		FileController.fileDirList("./data");
...
class FileController {
	...
	/*
	 * Binary file read write, 텍스트 파일 copy 해도 정상동작
	 */
	...
	static void fileDirList(String path) throws IOException {
		...
		for(File file : flist) {
			if(file.isDirectory()) {
				...
				//System.out.println(file.getPath());				// .\data\DS_Sample1.txt 경로까지 표시
			}
		}
	}

	/*
	 * search file, 하위경로까지 검색
	 * return : file name(with path), 시작경로 fileSearch("DS_Sample1.txt", "./data")
	 */
